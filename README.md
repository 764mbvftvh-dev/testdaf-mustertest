...
本仓库正在从“单一出题 Web 应用”演进为三个通过文件通信的本地系统：

- **出题系统**：生成 TestDaF 听力、阅读、写作、口语题目包，写入 `question_bank/`。
- **学生答题系统**：只读 `question_bank/`，后续写入 `student_attempts/`。
...
flowchart LR
    ...
    Student -->|未来写作答包| SA[(student_attempts/)]
...
shared/                  # 三系统共享的文件工具和题库只读 reader
docs/                    # 架构说明、路线图、题型分析
scripts/                 # 诊断脚本
tests/                   # 回归测试
question_bank/           # 本地生成题库，不纳入版本控制
student_attempts/        # 未来学生作答目录
...
- 出题系统写 `question_bank/`。
- 学生系统只读 `question_bank/`，未来写 `student_attempts/`。
